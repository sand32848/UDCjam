...
namespace WorldSpaceTransitions
{
    ...
    public class NoiseTextureWindow : EditorWindow
    {
        ...
        [SerializeField]
        TextureSpace textureSpace = TextureSpace.texture3D;
        [SerializeField]
        int resolution2d = 1024;
        [SerializeField]
        int resolution3d = 256;
        [SerializeField]
        int resolution3d3d = 256;

        private Texture2D noiseTex;
        private Texture3D noiseTex3D;
        ...
        void OnEnable()
        {
            ...
            if (EditorPrefs.HasKey("resolution3d3d")) resolution3d = EditorPrefs.GetInt("resolution3d3d");
        ...
        void OnGUI()
        {
            ...
            if (noiseType == 0 || (int)noiseType == 1)
            {
            ...
            /*if((int)textureSpace==2) resolution3d = EditorGUILayout.IntSlider("texture3d size", resolution3d, 2, 256);
            //limit atlas textures to squares
            int sqrt = Mathf.RoundToInt(Mathf.Pow(resolution3d,0.25f));
            resolution3d = sqrt*sqrt*sqrt*sqrt;*/
            if ((int)textureSpace == 1) resolution3d3d = EditorGUILayout.IntSlider("texture3d size", resolution3d3d, 2, 512);
            //
            if (GUI.changed)
            {
                NoiseGenerator.noiseType = noiseType;
            ...
            if (GUILayout.Button("Generate Texture", GUILayout.Width(150)))
            {
                ...
                //if ((int)textureSpace == 2) noiseTex = NoiseGenerator.Generate3DAtlas(boxSize, deepness, delta, resolution3d);
                if ((int)textureSpace == 1) noiseTex3D = NoiseGenerator.Generate3D(ref noiseTex, boxSize, deepness, delta, resolution3d3d);
            ...
            if (noiseTex)
            {
                ...
                if (GUILayout.Button("Save Texture", GUILayout.Width(120), GUILayout.Height(25)))
                {
                    ...
                    //
                    string path = "";
                    if ((int)textureSpace == 0)//|| (int)textureSpace == 2)
                    {
                        path = EditorUtility.SaveFilePanel("Save Noise Texture", tPath, tName + ".png", "png");
                    }
                    if ((int)textureSpace == 1)
                    {
                        path = EditorUtility.SaveFilePanelInProject("Save Noise Texture3D", tName + "_3D.asset", "asset", "enter a file name");
                    }
                    ...
                    if (path.Length > 0)
                    {
                        ...
                        if ((int)textureSpace == 1)
                        {
                            if (noiseType == NoiseType.pixel) noiseTex3D.filterMode = FilterMode.Point;
                            AssetDatabase.CreateAsset(noiseTex3D, path);
                            AssetDatabase.Refresh();
                            //save thumb image
                            path = path.Replace(".asset", "thumb.png");
                            byte[] bytes = noiseTex.EncodeToPNG();
                            File.WriteAllBytes(path, bytes);
                            AssetDatabase.Refresh();
                            TextureImporter A = (TextureImporter)AssetImporter.GetAtPath(path);
                            A.textureType = TextureImporterType.Sprite;
                            //A.spriteImportMode = SpriteImportMode.Single;
                            AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceUpdate);
